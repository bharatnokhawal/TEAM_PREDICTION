...
fantasy_points_list = []
for points, player in top_11_players:
    fantasy_points_list.append((points, player))

# Add impact scores to fantasy points
for i, (points, player) in enumerate(fantasy_points_list):
    # Calculate impact score for each player (you can modify this calculation as needed)
    impact_score = points * 0.1  # Example: 10% of fantasy points as impact score
    adjusted_points = points + impact_score
    fantasy_points_list[i] = (adjusted_points, player)

# Sort players based on adjusted fantasy points
fantasy_points_list.sort(reverse=True, key=lambda x: x[0])

# Print top 11 players based on adjusted fantasy points
print("Top 11 players based on adjusted fantasy points (including impact scores):")
for points, player in fantasy_points_list[:11]:
    print(f"Player name: {player} - Adjusted Fantasy Points: {points:.2f}")
